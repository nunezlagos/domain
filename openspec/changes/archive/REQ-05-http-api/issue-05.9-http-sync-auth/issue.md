...
- **Módulos sospechados:** `internal/api/sync.go` (sync status handler), `internal/api/middleware.go` (auth middleware), `internal/sync/status.go` (sync state tracking, issue-07.3), `cmd/engram/` (env var loading)
- **Riesgos / dependencias:** Depende de issue-07.3 (sync status) para los datos de sync. El auth middleware debe aplicarse a rutas específicas de otras HUs.
