...
- **Riesgos / dependencias:** Depende de issue-06.5 (pgvector) para embeddings. La generación de embedding debe ser async o fallar gracefulmente si no hay provider LLM configurado. El JSON Schema debe validarse server-side con una lib como `go-playground/validator` o `jsonschema`.
