# Design: issue-04.5-traceability
...
type TraceabilityService interface {
    ...
    GetHUsWithoutProposals() ([]Issue, error)
    GetHUsWithoutDesigns() ([]Issue, error)
    GetHUsWithIncompleteTasks() ([]Issue, error)
...
FROM issues us
LEFT JOIN proposals p ON p.issue_id = us.id AND p.version = (SELECT MAX(version) FROM proposals WHERE issue_id = us.id)
LEFT JOIN designs d ON d.issue_id = us.id AND d.version = (SELECT MAX(version) FROM designs WHERE issue_id = us.id)
...
```
Forward Traceability:
  REQ-01 ──→ issue-01.1 ──→ Proposal v2 ──→ Design v1 ──→ Tasks (3/5) ──→ [file1.go, file2.go]
        ──→ issue-01.2 ──→ Proposal v1 ──→ (no design) ──→ Tasks (0/2)
        ──→ issue-01.3 ──→ (no proposal)

Backward Traceability:
  file.go ──→ code_references ──→ issue-01.1 ──→ REQ-01
