# HU-04.10-lifecycle-state-tracking
...
HU-04.8 trackea `intake_payloads.status`. HU-04.9 trackea `external_sync_state.sync_status`. Las HUs y REQs tienen su `status` propio. Esta HU **unifica** todas en un view consultable + agrega:
...
Dado que entidad lleva > umbral configurado en mismo estado:
  ...
  user_stories.status="in_progress" > 30 días sin commit
...
```gherkin
Dado que existe intake INT-123 → HU-12.3 → sync con DIDE-145
...
-- Seed inicial con rules para intake/req/hu/sync_state via HU-01.7
...
CREATE OR REPLACE VIEW lifecycle_overview AS
  ...
  FROM requirements r
  LEFT JOIN user_stories hu ON hu.req_id = r.id
...
- **Dependencias hard:** HU-04.1, HU-04.2, HU-04.8 (intake states), HU-04.9 (sync states), REQ-02.2 (RBAC), REQ-02.4 (audit)
