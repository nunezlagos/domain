# Design: issue-04.8-intake-pipeline
...
BEGIN;
  ...
  INSERT INTO issues (req_id=..., ...) RETURNING id;
  UPDATE attachments SET issue_id=... WHERE staged_intake_id=... ;
  -- si action=merge
  UPDATE issues SET description_md=..., gherkin_text=... WHERE id=:target;
  UPDATE attachments SET issue_id=:target WHERE staged_intake_id=... ;
  -- siempre
  INSERT INTO intake_to_req_links (...);
  UPDATE intake_payloads SET status='committed', resulting_req_id=..., resulting_issue_id=...;
