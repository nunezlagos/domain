...
func (h *sddReviewHandler) Build(_ context.Context, in Input) (*Output, error) {
	...
	fmt.Fprintln(&b, "   cada item con domain_verify_update_item(verification_id, project_slug,")
	fmt.Fprintln(&b, "   status=pass|fail|skipped, output=evidencia). project_slug es requerido: sin él")
	fmt.Fprintln(&b, "   el checkpoint no se resuelve (DOMAINSERV-217).")
	fmt.Fprintln(&b, "   NO modifiques código — esta fase es read-only.")
	fmt.Fprintln(&b, "4. Cierra con domain_verify_complete(verification_id, project_slug).")
