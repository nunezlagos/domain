"""Mantenedor de Plantillas de Agentes (agent_templates) — patrón consolidado `core`.

Reusa core.models (BaseModel: la tabla agent_templates NO tiene deleted_at, así
que NO hay soft-delete), core.service.MaintainerService (list + signal),
core.views.MaintainerViews (las 7 vistas estándar, de las cuales este mantenedor
...
IMPORTANTE — colisión de tabla con maintainers.agents:
    La tabla `agent_templates` YA está mapeada por maintainers.agents.models.
    AgentTemplate (modelo READ-ONLY embebido en el detalle de Agentes).
    Declarar acá un SEGUNDO modelo managed sobre la MISMA db_table dispara
    models.E028 (hard error de system checks: dos modelos managed → misma tabla)
    y, bajo el runner de tests que flipea managed=True, rompería el arranque de
    ...
    están EXENTOS de E028 y de la creación de tabla en syncdb. El proxy agrega
