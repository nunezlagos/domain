# Proposal: HU-04.2-user-stories-gherkin

## Intención

Implementar gestión de historias de usuario (HUs) con escenarios Gherkin almacenados como structured data, vinculados a requisitos (REQs). Los escenarios se almacenan con campos separados (feature, scenario, given[], when, then[]) para permitir consultas y trazabilidad granular.

## Scope

**Incluye:**
- Tabla `user_stories` con: `id` (UUID), `slug` (TEXT UNIQUE, formato "HU-NN.N-slug"), `title`, `description`, `status` (proposed|active|completed|archived), `priority`, `req_id` (FK a requirements), `created_at`, `updated_at`
- Tabla `gherkin_scenarios` con: `id` (UUID), `hu_id` (FK), `feature` (VARCHAR), `scenario` (VARCHAR), `given` (TEXT[]), `when` (TEXT), `then` (TEXT[]), `position` (INT), `created_at`
