...
La tabla `agent_templates` (managed=False; Django solo lee/escribe via ORM) YA
esta mapeada por maintainers.agents.models.AgentTemplate, donde se declaran las
...
core.models.BaseModel a traves del modelo concreto de agents (que hereda de
...
Por que PROXY y no un segundo modelo concreto:
    Dos modelos `managed` sobre la MISMA db_table disparan models.E028 (system
    check error), y el runner de tests —que flipea managed=False→True— haria
    ...
    estan exentos). Asi NO tocamos maintainers.agents ni core, y este app suma
...
class AgentTemplate(_AgentTemplate):
    ...
    display_name y los metodos get_*_display del modelo concreto.
    """
