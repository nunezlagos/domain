# HU-04.9-external-provider-sync
...
Toda edición canónica de `requirements.title/description` y `user_stories.title/description/gherkin` ocurre en Domain (vía MCP, CLI o Web UI). Jira es mirror: refleja lo que Domain dice. Si alguien edita en Jira:
...
Y se uploadean los attachments (HU-04.6) uno por uno + PUT description con media inline IDs
...
Cuando se persiste el cambio en user_stories
...
Y actualiza `user_stories.status` según mapping:
  - "To Do" → "approved"
...
Entonces:
  - "accept_external" → Domain copia los valores de Jira al user_stories + actualiza last_pushed_values, sync_status="ok"
...
Entonces se crea un SEGUNDO external_sync_state row para el mismo hu_id pero distinto provider
...
CREATE TABLE external_sync_state (
  ...
  entity_id UUID NOT NULL,                  -- FK soft a requirements.id o user_stories.id
...
- **Dependencias hard:** HU-04.1, HU-04.2, HU-04.6, REQ-02.3 (secrets para credentials), REQ-02.4 (audit), REQ-10.2 (webhook receiver con HMAC), REQ-26.5 (circuit breaker)
