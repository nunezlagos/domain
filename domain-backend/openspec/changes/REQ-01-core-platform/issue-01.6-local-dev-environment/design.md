# Design: issue-01.6-local-dev-environment
...
> Nota: este script solo corre la primera vez que se inicializa el volumen `domain_pg_data`. Las extensiones también se deben crear vía migración golang-migrate (issue-01.1) para entornos no-Docker. La redundancia es intencional y segura (`IF NOT EXISTS`).
