...
// AdaptiveService es la API v2 del wizard (HU-04.7 reemplaza v1 de 8
// preguntas fijas). El cliente solo necesita StartAdaptive + AnswerAdaptive.
//
// Persiste el envelope en hu_drafts.answers["__envelope__"] y los slots
// provided en hu_drafts.answers[slot_key].
...
func (a *AdaptiveService) StartAdaptive(ctx context.Context, rawPrompt string, createdBy *uuid.UUID) (*Draft, *wp.Question, error) {
	...
	if errors.Is(qErr, wp.NoMoreQuestionsErr) {
		...
		_, _ = a.Pool.Exec(ctx,
			`UPDATE hu_drafts SET status = $1, current_step = total_steps, updated_at = now() WHERE id = $2`,
			StatusFinished, d.ID,
		)
...
func (a *AdaptiveService) persistEnvelope(ctx context.Context, draftID uuid.UUID, env *wp.ContextEnvelope) error {
	...
	row := a.Pool.QueryRow(ctx,
		`SELECT COALESCE(answers, '{}'::jsonb) FROM hu_drafts WHERE id = $1`, draftID)
