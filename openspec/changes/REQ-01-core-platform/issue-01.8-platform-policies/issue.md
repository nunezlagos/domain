...
Y respeta timeout 2s + circuit breaker (issue-12.6)
...
Cuando el binary boot:
  1. golang-migrate up
  2. seeders run (issue-01.7) → import .md embebidos a DB con UPSERT
...
Y linters issue-25.13/issue-13.9 leen desde body_structured (no parsean markdown raw)
```
