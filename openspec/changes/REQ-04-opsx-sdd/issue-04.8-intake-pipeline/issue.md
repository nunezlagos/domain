...
## Diferencia con issue-04.7

| issue-04.7 wizard | issue-04.8 intake |
|---|---|
...
```
        ...
        └──────┬───────┘     espera approve/edit/reject/merge
               ▼
        ┌──────────────┐
        │ 6. commit    │  crea requirements + issues + attachments
        └──────┬───────┘     status=committed
               ▼
        ┌──────────────┐
        │ 7. external  │  dispara issue-04.9 (sync a Jira u otro provider)
        └──────────────┘
```
...
Y se persisten en `intake_payloads.dedup_candidates` como `[{req_id, issue_id?, similarity, reason}]`
...
Y el draft NO crea row en `issues` todavía (solo el draft en intake_payloads)
```
...
Entonces dentro de UNA transaction se crea:
  ...
  - row en issues con title, description, gherkin del draft
  - rows en attachments con los S3 keys ya subidos en step 1
  - row en intake_to_req_links con link_type="created"
Y se actualiza intake_payloads.status="committed", resulting_req_id, resulting_issue_id
Y se dispara evento "req.hu.created" al event bus (REQ-10.3)
Y NO se llama todavía a Jira (eso es issue-04.9 disparado por el evento)
```
...
Cuando humano invoca `domain_intake_approve({intake_id, action: "merge", target_issue_id: "..."})`
...
Entonces cada attachment se sube a S3 vía issue-04.6 al bucket "intake-staging/{intake_id}/"
...
CREATE TABLE intake_payloads (
  ...
  dedup_candidates JSONB NOT NULL DEFAULT '[]',
    -- [{ req_id, issue_id?, similarity, reason }]
  ...
  human_review_target_issue_id UUID,
  ...
  resulting_issue_id UUID REFERENCES issues(id),
...
CREATE INDEX ON intake_to_req_links (issue_id) WHERE issue_id IS NOT NULL;
...
| `domain_intake_approve` | `{intake_id, action: "create_new"\|"merge", target_issue_id?, edits?}` | `{resulting_req_id, resulting_issue_id, audit_log_id}` |
...
- **Dependencias hard:** issue-04.1 (REQ/HU CRUD), issue-04.2 (Gherkin), issue-04.6 (S3), issue-04.9 (Jira sync — disparada por evento, no bloqueante), REQ-06 (LLM), REQ-05 (skills), REQ-03 (memory para dedup embeddings), REQ-10.3 (event bus)
- **Adapters deferred:** issue-04.8a email, issue-04.8b webhook, issue-04.8c sheets, issue-04.8d slack (futuras, declaran interface aquí)
