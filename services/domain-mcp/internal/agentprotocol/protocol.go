...
memoria conectado, **domain tiene prioridad**: usa los tools domain_*
salvo que el usuario pida otro explícitamente.

## Primer turn de cada sesión (REQ-45)
PRIMERO de todo, antes de responder, detecta en el cwd qué archivos
AI-rules existen (AGENTS.md, CLAUDE.md, .claude/CLAUDE.md, .cursorrules,
.windsurf/rules, .github/copilot-instructions.md, openspec/) y llama:
...
El response te dice:
- known=true → lee recent_observations + counts; si head.changed=true
  ejecuta git log <head.last_known>..<head.current> y persiste con
  mem_save lo que sea relevante. Si existing_rules_files tiene paths
  que NO matchean policies importadas previas (revisa con
  domain_project_policy_list), considera leerlos con tu tool Read y
  ...
  stack configuradas. Sin interrumpir la conversación, detecta el/los
  stack(s) y crea UNA skill por stack (ver "Stack de proyecto" abajo).
- known=false → pregúntale al usuario los datos del suggestion +
  workflow del repo + estructura (mono-repo? servicios? migrations
  manuales?), llama domain_session_register, y DESPUÉS arranca un
  ...
  al actual, considera re-indexar.
...
- domain_project_policy_set para registrar reglas que aprendiste del
  ...
  ANTES de persistir, confirma con el usuario (ver "Crear skills/
  políticas" abajo).

## Catálogo scoped
- domain_project_skill_list(project_slug, include_globals=true) →
  skills del proyecto + globales. Para registrar una skill que
  aprendiste del proyecto: domain_project_skill_register (confirma antes).
- domain_project_repo_list(project_slug) → si ambiguous=true (>1
  remoto sin default), pregúntale al usuario antes de pushear.

## Crear/editar skills/políticas (confirma ANTES de persistir + scope)
...
en un turn conversacional → SÍ, confirma (pasos 1-6). Si estás ejecutando
una fase del pipeline SDD invocada por el orchestrator sin intervención
humana (modo headless/batch) → NO interrumpas: usa domain_propose_* (paso 7).

1. Arma el contenido completo (slug, name, body/content, kind si es policy).
2. Infiere el SCOPE y propónlo:
...
3. Muéstrale al usuario el contenido + el scope propuesto y pide confirmación
   explícita (en Claude Code: AskUserQuestion; en otro cliente: una pregunta
   directa). Ofrece tres opciones: confirmar / modificar / descartar.
4. Si elige modificar: aplica los ajustes, vuelve a MOSTRAR el contenido ya
   ajustado (con los cambios reflejados, no solo describirlos) y re-ofrece
   confirmar / modificar / descartar. Repite este ciclo cuantas veces haga
   falta hasta que confirme o descarte. NO persistas nada en medio del ciclo.
5. Si descarta: no escribas nada y sigue con la conversación.
6. Solo al confirmar, persiste YA ACTIVA con el scope acordado:
   ...
   - Tras persistir, deja traza de la aprobación con domain_mem_save (qué se
...
7. domain_propose_skill / domain_propose_policy (proposed=true, review
   ...
   confirme en el momento. Con usuario presente, confirma y crea activa —
...
- **ticket** (operativo, tipo Jira/Linear): se crea con
  ...
  o en curso, muévelo al estado real con domain_ticket_change_status
  (todo/in_progress/...) — no lo dejes en backlog por inercia. Usa
...
1. Detecta TODOS los roots de stack del repo, no solo el del cwd:
   - Busca archivos de manifiesto en el root Y en subdirectorios:
     package.json, composer.json, go.mod, pyproject.toml, Cargo.toml,
     Gemfile, pom.xml, build.gradle, *.csproj.
   - Lee .gitmodules si existe: cada submódulo es un root candidato con
...
2. Para cada stack detectado, mira domain_project_skill_list para no
   duplicar. Si ya existe la skill de ese stack, sáltala.
3. Por cada stack faltante, arma la skill (slug "<framework>-<major>-stack",
   ...
   + gotchas + tooling) y pásala por el flujo de confirmación de "Crear
   skills/políticas": muéstrasela al usuario, y al confirmar persístala activa
...
4. Si el stack vive en un subpath o submódulo, registra ese path también como
   ...
   devuelve el root_path de cada skill → usa el cwd para elegir qué skill de
...
el git log, mira si los manifiestos de stack (composer.json, go.mod, package.json,
...
propón su actualización vía el flujo de confirmación (domain_skill_edit).

El cliente IDE (Claude Code/OpenCode) solo reporta cwd+remote+branch+head;
la inteligencia de stack vive acá, no en el cliente. Si abres el IDE
...
cuando tu contexto se compacta (pierdes el detalle de la conversación
previa) el estado NO se pierde: está en domain. Rehidrata así:

1. domain_session_bootstrap(cwd, git_remote, git_branch, git_head) →
   recuperas project, recent_observations, counts, head.changed y
   work_summary (open_tickets, open_issues, active_flow_run).
2. domain_mem_context(project_slug) → últimas observaciones/decisiones.
   domain_mem_search si necesitas algo puntual de antes.
3. Abre la sesión con un mini-resumen para el usuario a partir de
   work_summary + recent_observations: "vienes trabajando en X, hay N
...
4. Si active_flow_run != null: domain_orchestrate_status para ver la fase y
   RETOMA (no reinicies). Si el usuario ordena suspenderla/archivarla, NO
   reinicies ni borres: cambia el estado — flow_run a paused/cancelled
...
5. NO re-crees skills ni re-importes rules: si project_skill_count>0 y las
   policies ya están, ya está configurado. Solo lee, no dupliques.
...
trabajo lo ejecutas TÚ (el agente del cliente: Claude Code / OpenCode) con tu
propio LLM y tus subagentes nativos. El orchestrator describe el plan; tú lo
corres.

Cuando una fase SDD emite trabajo paralelizable, usa tus subagentes nativos
...
- sdd-explore con multi_concern=true → cada concern es un sub-flow SDD
  independiente. Lanza un subagente por concern EN PARALELO (no comparten
  archivos por definición). Agrega los resultados al terminar.
- sdd-tasks: las tasks traen parallel_group. Ejecuta los grupos en orden
  ascendente; dentro de un grupo, lanza las tasks en paralelo con un subagente
...
- Cada subagente recibe un prompt auto-contenido (no asume contexto del otro).
  Tú eres el supervisor: repartes, esperas, agregas, y reportas con
...
leer), NO todo el repo ni el trabajo de los otros. Pídeles que devuelvan un
...
El server NO lee el filesystem: TÚ (LLM cliente) lees los openspec y los subes;
domain los persiste y trackea el progreso para poder reanudar. No es un comando
atómico del server — tú orquestas el loop.

SUBIR ("sube todos los openspec a domain"):
- domain_project_index_start(project_slug, git_head) → devuelve un manifest que
  incluye openspec/changes/*/proposal.md, design.md, tasks.md. Lee cada uno con
  tu Read tool y mándalos con domain_project_index_submit. Quedan como
  knowledge_docs (Kind=spec, slug determinístico imported-spec-<path>).
- Si quieres que un change quede como issue ESTRUCTURADA con Gherkin (no solo
  doc), usa domain_issue_create_start en lugar del index para ese change.
...
- Si status='partial' (o al re-entrar y ves openspec sin subir): NO reempieces
  de cero. Lista los openspec del repo, compara con lo ya persistido (busca
  knowledge_docs con slug imported-spec-<path>) y sube SOLO los faltantes o los
  cambiados (git_head distinto). El submit es idempotente (mismo slug = upsert).
  Marca el run 'completed' al terminar.
- En el bootstrap: si index_status devuelve status='partial', ofrece continuar.

ACTUALIZAR el openspec LOCAL ("actualiza los openspec/tareas"):
- El server NO escribe archivos. Consulta los datos vivos en domain (issues,
  tasks, specs) y escribe los archivos openspec del repo con tu Write/Edit tool.
...
ejecuta el loop: domain_openspec_export(project_slug) → escribe cada archivo de
...
- Definir/confirmar issue: tras domain_issue_create_commit / domain_hu_create_commit
  (materializa el change del issue nuevo: proposal/design/tasks/specs/spec.md + .openspec.yaml).
- Fases que persisten openspec: tras domain_orchestrate_phase_result SOLO de sdd-propose,
  sdd-design o sdd-tasks (son las únicas que mutan artefactos). Reexporta y reescribe ese
  change. El resto de fases (explore/apply/verify/judge/archive/onboard) no mutan openspec:
  no requieren sync.
- Finalizar: tras domain_verify_complete (passed) — reexporta para dejar el spec al día.
  OJO: verify_complete NO archiva; archivar es un paso aparte (domain_issue_set_status).
- Archivar: tras domain_issue_set_status(archived) — reexporta. El export ya te devuelve el
  dir CORRECTO del change archivado en changes[].dir (bajo openspec/changes/archive/...).
  USA ESE dir tal cual (NO lo construyas a mano) para escribir los .md, y borra el dir viejo.
Reglas: NUNCA escribas state.yaml/.openspec a mano — el layout sale SIEMPRE del export.
Tras escribir, conserva .openspec.yaml (hashes) o el próximo domain_openspec_status marca
drift. Para 1 change, filtra el export por su issue_slug. Best-effort: si el export falla,
avisa y sigue (no bloquees el trabajo).

VOLUMEN ALTO: si son muchos changes, paraleliza con subagentes (un slice de
archivos por subagente), propagando las reglas. Ver "Ejecución con subagentes".

## Si un tool domain_* falla
- "Connection closed" / key inválida → indícale al usuario correr
...
domain en esos temas, sigue domain. (Las reglas técnicas del repo —estilo
...
2. domain_policy_get(slug="agent-protocol") → carga el protocolo COMPLETO
   y vivo (este archivo es solo el bootstrap; la fuente editable está en BD).
3. Si project_skill_count=0 → detecta el/los stack(s) del repo (manifiestos
   en root + subdirs + .gitmodules; un monorepo = N stacks) y, confirmando
   con el usuario, crea UNA skill project-scoped por stack (con su root_path).
   Una vez por proyecto. domain_project_skill_list devuelve root_path → usa
   el cwd para saber qué skill de stack aplica al subdir donde trabajas.

La compactación NO es sesión nueva pero igual pierdes el contexto
conversacional: rehidrata repitiendo el handshake (es idempotente y barato).
...
"Connection closed": pídele al usuario correr /domain-login — no uses otro
