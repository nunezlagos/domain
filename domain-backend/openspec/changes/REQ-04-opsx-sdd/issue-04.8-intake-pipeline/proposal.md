...
**No incluye:**
- Adapter de email (issue-04.8a futura)
- Adapter de webhook HTTP genérico (issue-04.8b futura)
- Adapter de Excel/Google Sheets (issue-04.8c futura)
- Adapter de Slack (issue-04.8d futura)
- Sync hacia Jira/GitHub/Linear (issue-04.9)
- State tracking unificado por entidad (issue-04.10)
...
**Por qué no:** se pierde auditoría, trazabilidad SDD, multi-tenant scoping, dedup semántico contra el catálogo interno. Domain debe ser source of truth (decisión confirmada con el usuario). Jira es mirror downstream (issue-04.9).
...
- issue-04.2 — user-stories + Gherkin storage
- issue-04.6 — S3 attachments
...
- issue-04.9 — external sync (intake.committed dispara sync; sin 04.9 el evento queda sin consumer)
- issue-04.10 — state tracking unificado (intake state se ve solo en su tabla)
...
- E2E: agent submits → polls → approves → row en issues existe + attachments referenciadas correctamente
