...
// HUDedupSource compara el prompt vs user_stories existentes via FTS sobre
// title/description. (Implementación naive — versión vector pendiente para
// cuando el embedder sea wired al wizard.)
type HUDedupSource struct {
	Pool  *pgxpool.Pool
	Limit int // default 5
}

func (s *HUDedupSource) Name() string { return "hu_dedup" }

func (s *HUDedupSource) Run(ctx context.Context, env *wizardplan.ContextEnvelope) error {
	if s.Pool == nil {
		return nil
	}
	limit := s.Limit
