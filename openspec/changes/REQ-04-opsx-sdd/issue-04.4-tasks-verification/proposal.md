# Proposal: issue-04.4-tasks-verification
...
**Incluye:**
- Tabla `tasks` con: `id`, `issue_id` (FK), `section` (VARCHAR), `description` (TEXT), `status` (pending|in_progress|completed), `position` (INT), `started_at`, `completed_at`, `completed_by`, `created_at`, `updated_at`
...
4. **Progress query**: `SELECT issue_id, COUNT(*) as total, COUNT(*) FILTER (WHERE status = 'completed') as completed FROM tasks GROUP BY issue_id`
