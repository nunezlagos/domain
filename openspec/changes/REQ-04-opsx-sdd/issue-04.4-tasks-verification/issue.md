...
Feature: Tasks and Verification Management
  ...
  Scenario: Crear tareas para una HU
    When creo tareas para "issue-01.1-db-schema":
      | section  | description                         |
...
- **Qué pide realmente:** 3 tablas: `tasks`, `verification_results`, `sabotage_records`. FK a issues. Status tracking (pending → in_progress → completed). Progress calculation.
- **Módulos sospechados:** `internal/opsx/task.go`, `internal/store/pg/task.go`
- **Riesgos / dependencias:** Depende de issue-04.2. Verification y sabotage son extensibles (polimórficos).
