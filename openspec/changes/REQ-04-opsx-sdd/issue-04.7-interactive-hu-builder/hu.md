# issue-04.7-interactive-hu-builder
...
2. ¿Audience destinataria? (de catálogo issue-01.9)
...
**Todo el state vive en BD** (tabla `issue_drafts`), consistente con el principio "todo en BD" del proyecto. El MCP/CLI/futura Web UI son clientes del orquestador.
...
Entonces se crea draft row en `issue_drafts` con id UUID
Y se devuelve `{draft_id, next_question: {key, prompt, options[]}, progress: "1/8"}`
Y la primera pregunta es "¿Qué audience principal se beneficia?" con opciones del catálogo issue-01.9
```
...
Entonces se persiste la respuesta en issue_drafts.answers JSONB
...
Y se devuelve `{preview: {...}, suggested_slug: "issue-XX.Y-name", target_path: "openspec/changes/REQ-XX-name/issue-XX.Y-name/"}`
...
```gherkin
Dado que existe tabla `audiences` (issue-01.9) seedeada con 10 audiences
...
Entonces propone "openspec/changes/REQ-03-memory-system/issue-03.{next}-{slug}"
...
Y el state issue_drafts queda como audit trail
```
...
CREATE INDEX ON issue_drafts (created_by, status);
CREATE INDEX ON issue_drafts (expires_at) WHERE status = 'in_progress';

CREATE TABLE hu_draft_steps_log (
  id BIGSERIAL PRIMARY KEY,
  draft_id UUID NOT NULL REFERENCES issue_drafts(id) ON DELETE CASCADE,
...
| `domain_issue_drafts_list` | `{status?}` | `{drafts[]}` |
...
- **Módulos sospechados:** internal/sdd/wizard, internal/mcp/tools/sdd, schema migration issue_drafts
