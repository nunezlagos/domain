...
export const MAINTAINERS: MaintainerDef[] = [
  // === CORE (autenticación, identidad, sesiones) ===
  {
    ...
    columns: [
      ...
      { key: 'role', label: 'Rol' },
      { key: 'organization_id', label: 'Org', value: r => truncate(r.organization_id, 12) },
  ...
  },
  {
    ...
    columns: [
      ...
      { key: 'user_id', label: 'User', value: r => truncate(r.user_id, 8) },
  ...
  // === RESOURCES (skills, agents, flows, crons, prompts) ===
  {
    ...
    columns: [
      ...
      { key: 'project_id', label: 'Project', value: r => r.project_id ? truncate(r.project_id, 8) : 'global' },
  ...
  },
  {
    ...
    columns: [
      ...
      { key: 'enabled', label: 'Enabled', value: r => r.enabled ? 'Sí' : 'No',
        format: 'badge', badgeColor: statusColor },
      { key: 'created_at', label: 'Creado', format: 'date' },
    ],
  },
  ...
  },
  {
    ...
    columns: [
      { key: 'filename', label: 'Filename', nullable: false },
      { key: 'content_type', label: 'Content-Type' },
      { key: 'size_bytes', label: 'Size', value: r => r.size_bytes ? `${(r.size_bytes/1024).toFixed(1)}KB` : '—' },
  ...
  // === OBSERVABILITY (audit, memory, prompts capturados) ===
  {
    ...
    columns: [
      ...
      { key: 'origin_org_id', label: 'Org', value: r => truncate(r.origin_org_id, 8) },
    ],
  },
  {
  ...
  },
  {
    ...
    columns: [
      ...
      { key: 'status', label: 'Status', format: 'badge', badgeColor: statusColor },
      { key: 'priority', label: 'Prioridad' },
      { key: 'project_slug', label: 'Project' },
      { key: 'updated_at', label: 'Actualizado', format: 'date' },
    ],
  },
];
