...
- **Módulos sospechados:** `internal/api/` (nuevo paquete HTTP handler), `internal/store/sessions.go` (repo de sesiones, puede existir de HU-02.1), `cmd/engram/` (entrypoint serve)
- **Riesgos / dependencias:** Depende de HU-02.1 (session start/end) para la capa de store; delega en la store layers existentes; requieres sesiones en DB
