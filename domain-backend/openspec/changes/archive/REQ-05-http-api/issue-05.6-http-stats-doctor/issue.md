...
- **Módulos sospechados:** `internal/api/stats.go` (handlers), `internal/store/stats.go` (queries de agregación), `internal/doctor/` (diagnóstico, issue-12.1), `internal/version/` (build info)
- **Riesgos / dependencias:** Depende de issue-12.1 (doctor readonly checks) y issue-12.3 (health/version). Stats queries deben ser eficientes.
