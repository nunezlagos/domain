...
- **Módulos sospechados:** `internal/api/search.go` (handlers), `internal/store/search.go` (FTS5 queries, HU-01.3), `internal/store/observation.go` (context/timeline queries)
- **Riesgos / dependencias:** Depende de HU-01.3 (FTS5 search) y HU-01.2 (observation queries). FTS5 debe estar configurado con triggers.
