# Design: issue-01.5-topic-key-upsert

## Decisión arquitectónica

### Método directo en Store (no wrapper)

A diferencia de la deduplicación (issue-01.4) que es un cross-cutting concern implementado como wrapper, el upsert por topic_key es **parte natural del CRUD de observaciones**. Se implementa como un método directo en la interfaz `Store`:
...
2. **Composabilidad con wrappers** — `DeduplicatingStore` (issue-01.4) puede llamar a `base.UpsertByTopicKey` si el observation tiene topic_key, o a `base.SaveObservation` si no
...
type SaveResult struct {
    ...
    Deduplicated bool     // issue-01.4
    OriginalID   int64    // issue-01.4 (ID del original si dedup)
    Updated      bool     // issue-01.5 (true si fue upsert)
}
```
...
### Interacción con deduplication (issue-01.4)
...
| Mezcla de dedup + upsert (issue-01.4 + issue-01.5) | El wrapper de dedup debe detectar topic_key != "" y delegar a UpsertByTopicKey en lugar de aplicar dedup, porque upsert es semanticamente diferente |
