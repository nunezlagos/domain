# Design: HU-04.3-specs-designs

## Decisión arquitectónica

**Versionado por INSERT (append-only) en lugar de UPDATE. Cada versión es un registro completo. "Última versión" = MAX(version) GROUP BY hu_id.**
...
FROM proposals WHERE hu_id = $1;
```

**Índices:**
- `proposals_hu_id_version_idx` UNIQUE BTREE (hu_id, version)
- `proposals_status_idx` BTREE (status)
- `designs_hu_id_version_idx` UNIQUE BTREE (hu_id, version)
...
```
user_stories (1) ──→ (N) proposals (1) ──→ (N) designs
     │                      │                     │
