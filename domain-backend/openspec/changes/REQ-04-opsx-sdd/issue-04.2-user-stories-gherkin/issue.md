...
**Quiero** gestionar issues (HUs) con campos estructurados y criterios de aceptación en Gherkin
...
Feature: User Stories with Gherkin Scenarios
  ...
  Scenario: Slug único por REQ
    When intento crear "issue-01.1-db-schema" dentro del mismo REQ
    Then recibo error de UniqueViolation (slug único global)
```

## Análisis breve

- **Qué pide realmente:** Tabla `issues` + tabla `gherkin_scenarios` (1:N). Escenarios con campos estructurados feature/scenario/given/when/then. FK a requirements.
- **Módulos sospechados:** `internal/opsx/user_story.go`, `internal/store/pg/user_story.go`
- **Riesgos / dependencias:** Depende de issue-04.1 (requirements table). Gherkin como structured data es más complejo que texto plano pero más útil para trazabilidad.
