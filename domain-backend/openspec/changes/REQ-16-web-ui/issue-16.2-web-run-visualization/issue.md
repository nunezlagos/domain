# issue-16.2-web-run-visualization
...
- **Qué pide realmente:** Frontend de visualización de runs con DAG interactivo, logs streaming (SSE), métricas de paso, timeline view. Backend: SSE endpoint para streaming de logs (issue-11.3).
- **Módulos sospechados:** `web/` (frontend), `internal/api/handlers/runs.go` (SSE endpoint)
- **Riesgos / dependencias:** Depende de issue-11.3 (execution streaming), issue-09 (flow system), issue-08 (agent system). DAG rendering library.
