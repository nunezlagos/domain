...
func TestIssueSlugValidation(t *testing.T) {
	valid := []string{
		"issue-01.1",
		"issue-01.1-db-schema",
		"issue-99.99-auth",
	...
	for _, s := range valid {
		require.True(t, reIssueSlug.MatchString(s), "slug %q debe ser válido", s)
	}
	for _, s := range invalid {
		require.False(t, reIssueSlug.MatchString(s), "slug %q debe ser inválido", s)
	}
}
...
func TestCreateValidation(t *testing.T) {
	...
	_, err = s.Create(nil, "issue-01.1", "", "", "", "", "REQ-01", nil)
	require.Error(t, err)

	_, err = s.Create(nil, "issue-01.1", "title", "", "bogus", "", "REQ-01", nil)
