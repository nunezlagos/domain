# Design: HU-04.9-external-provider-sync
...
- `accept_external`: copio Jira → Domain. Update `user_stories.{title,description,gherkin}` con valores Jira. `last_pushed_values` actualizado. `sync_status=ok`.
