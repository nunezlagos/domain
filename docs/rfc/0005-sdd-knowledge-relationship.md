...
**Status:** accepted
**Date:** 2026-06-07
**Related:** REQ-03 Memory System (HU-03.4 knowledge-documents), REQ-04 OPSX SDD

## Contexto

REQ-04 (OPSX SDD) modela artifacts: REQs, HUs (con Gherkin), specs, designs (ADRs), tasks.
REQ-03 HU-03.4 modela knowledge_docs: markdown documents con chunking, embeddings, búsqueda híbrida.
...
ALTER TABLE knowledge_docs
  ...
  ADD COLUMN sdd_slug VARCHAR(100);    -- "REQ-01", "HU-01.1"
...
```
REQ-01 (kind=sdd_req)
  └─ HU-01.1 (kind=sdd_hu, parent=REQ-01)
      ├─ HU-01.1 / hu.md      (kind=sdd_hu, structured)
      ├─ HU-01.1 / proposal   (kind=sdd_spec)
      ├─ HU-01.1 / design     (kind=sdd_design)
      ├─ HU-01.1 / tasks      (kind=sdd_tasks)
      └─ HU-01.1 / state      (kind=sdd_state)
```
...
Una sola búsqueda híbrida (HU-03.7) cubre todos:
...
- knowledge_docs ya tiene versioning (HU-03.4 implícito, formalizar si no está)
...
`requirements`, `user_stories`, `specs`, `designs`, `tasks_table`, `state_machines`.
...
Hoy `openspec/changes/REQ-*/HU-*/*.md` viven en disco. Estrategia de migración:
...
- Agregar columnas a knowledge_docs en migración (HU-03.4 ya base)
...
- HU-03.7 global search ya cubre todos los kinds

## Open questions

- ¿"State machine" como kind separado o columna en sdd_hu? — Yo voto columna en sdd_hu para evitar fragmentar
- ¿Adjuntos S3 (HU-04.6) atados a knowledge_docs.id genérico? Sí, polimórfico funciona
