"""Tests de las views (HTTP) del mantenedor de Flows.
...
class DetailViewTests(AuthenticatedMixin, TestCase):
    ...
    def test_detail_lista_versiones_readonly(self):
        ...
        # Las versiones no exponen acciones de mutación (sin data-action sobre ellas).
...
class EditViewTests(AuthenticatedMixin, TestCase):
    ...
    def test_post_edita_flow(self):
        ...
        r = self.client.post(reverse("flows:edit", args=[f.pk]), {
            ...
            "spec": "{}",
            "seed_version": "",
            # is_active omitido → checkbox unchecked → False
        })
        self.assertEqual(r.status_code, 302)
        f.refresh_from_db()
