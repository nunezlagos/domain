...
func (h *sddDesignHandler) Build(_ context.Context, in Input) (*Output, error) {
	...
	fmt.Fprintln(&b, "INTERACTIVO (REQ-55): ante decisiones de diseño abiertas (tradeoffs, "+
		"alternativas), CONSULTA al usuario con AskUserQuestion (opciones + 'Other' para "+
		"texto libre) y espera su respuesta. NO ejecutes esta fase en un subagente: "+
		"AskUserQuestion no está disponible ahí.")
	fmt.Fprintln(&b, "SYNC OBLIGATORIO (REQ-55): al terminar, corre domain_openspec_export, "+
		"escribe los .md en openspec/changes/<change>/ con tu Write tool, y domain_openspec_apply. "+
		"Reporta ambas en tool_calls o el server RECHAZA el cierre de la fase.")
	return &Output{
		...
		SuggestedSaves: []SuggestedSave{
			...
			{Type: "knowledge_doc", Required: true,
				Hint: "persiste el design como knowledge_doc para que quede registro del change en BD"},
		},
		SkillThreshold: 0,
