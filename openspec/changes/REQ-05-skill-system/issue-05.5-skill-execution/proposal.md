# Proposal: issue-05.5-skill-execution
...
**Excluye:**
- Sandboxing de código (se integrará con issue-11.1-sandbox-execution)
- Streaming de respuesta (issue-06.6)
- Reintentos automáticos (issue-09.4)

## Enfoque técnico

- **Strategy Pattern:** `Executor` interface con método `Execute(ctx, skill, params) → Result`. Implementación por tipo.
- **PromptExecutor:** Renderiza template con `text/template`, luego llama al LLM provider vía issue-06.2.
- **CodeExecutor:** Delega a sandbox (issue-11.1) o ejecuta en proceso aislado.
...
- **Ejecución de código arbitrario:** Riesgo de seguridad severo. Mitigación: sandbox obligatorio (issue-11.1) o ejecución en container efímero.
