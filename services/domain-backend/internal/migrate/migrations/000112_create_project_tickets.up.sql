-- migration: create_project_tickets
-- author: mnunez@saargo.com
-- issue: REQ-51 sistema de tickets internos (bugs/features/requirements)
-- description: BD = source of truth. Sync opcional con Jira/GitHub/GitLab
--   vía external_provider + external_id + external_url. Cada issue
--   pertenece a un proyecto; opcionalmente a un client del proyecto.
-- breaking: false
...
CREATE TABLE project_tickets (
  ...
  -- key: PROJ-1, PROJ-2 — generado al insertar. UNIQUE per project.
  -- El prefix se deriva del project.slug (uppercase, sin guiones).
  key             VARCHAR(40) NOT NULL,
  -- number: incremental dentro del proyecto. PROJ-1 → number=1.
  ...
  -- issue_type, status, priority — enums simples. Si en el futuro el
  -- usuario quiere workflows personalizados por proyecto, agregar tabla
  -- project_ticket_statuses configurable y mover status a FK.
  ...
  -- Sync externo (Jira/GitHub/GitLab issues). null = puramente interno.
...
-- Comments threads sobre cada issue.
CREATE TABLE project_ticket_comments (
  ...
  -- Si el comment viene de sync (Jira), guardar ref.
  external_id VARCHAR(80),
  created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  deleted_at  TIMESTAMPTZ
...
-- Status history para audit y métricas (cycle time, lead time, etc).
