# Design: issue-04.10-lifecycle-state-tracking
...
**State machine declarativa en BD**, no en código. Tabla `state_machine_rules` define `(entity_kind, from_state, to_state) → required_permission`. Permite ajustar reglas sin redeploy. Seedeado vía issue-01.7.
...
return lifecycle.Transition(ctx, "intake", intakeID, "committed",
    TransitionOpts{Reason: "human approved create_new", Context: {issue_id, req_id}})
```
...
-- hu in_progress > 30d
SELECT 'hu', id, status, last_transition FROM issues
...
| Append-only (no update audit) | GDPR right-to-erasure complica → solución: tomb `actor_id` con hash en lugar de borrar (issue-04.10 ↔ REQ-23.4) |
