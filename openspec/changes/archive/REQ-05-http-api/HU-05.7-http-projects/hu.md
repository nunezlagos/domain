...
- **Qué pide realmente:** (1) GET /project/current?cwd= — resuelve el nombre de proyecto desde un directorio usando la cadena de detección de HU-08.1. (2) POST /projects/migrate — mueve todas las observaciones/sessions/prompts de un proyecto a otro (UPDATE masivo). Requiere auth.
- **Módulos sospechados:** `internal/api/projects.go` (handlers), `internal/store/project.go` (project resolution), `internal/project/detect.go` (cadena de detección, HU-08.1), `internal/store/consolidation.go` (migración, HU-08.3)
- **Riesgos / dependencias:** Depende de HU-08.1 (project detection chain) y HU-08.3 (consolidation/migration). Auth via HU-05.9.
