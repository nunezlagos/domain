# Design: HU-01.2-observation-crud
...
Hard delete elimina físicamente; los triggers FTS5 (HU-01.1) se encargan de limpiar el índice automáticamente.

### FTS5 sync

No se toca FTS5 explícitamente. Los triggers definidos en HU-01.1 (`observations_ai`, `observations_ad`, `observations_au`) sincronizan automáticamente:
...
| Riesgo | Mitigación |
|--------|------------|
| Candidates lentos con muchas observaciones | Index on `normalized_hash` ya existe (HU-01.1); query es `WHERE normalized_hash = ?` que es O(log n) |
